...
names <- c('Venice', cities$name)
adjacencyMatrix <- matrix(0, nrow = 22, ncol = 22, dimnames = list(names, names))
adjacencyMatrix[2:22,1] <- cities$Cases 

layoutCoordinates <- rbind(c(x = 12.3155 , y = 45.4408), cbind(cities$longitude, cities$latitude))  # Get graph layout coordinates
...
case.size <- log(c(10,cities$Cases))
...
zp1 <- zp1 + geom_point(data = data.frame(layoutCoordinates),  # Add nodes
                        aes(x = x, y = y), size = case.size, pch = 21,
